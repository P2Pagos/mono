...
export default defineNuxtModule({
  meta: {
    ...
    configKey: 'p2payRobosatsFlow'
  },
  defaults: {
    enabled: true,
    routeBase: '/flows/robosats',
    apiPrefix: '/api/flows/robosats',
  ...
  setup(options, nuxt) {
    ...
    const prefix = String(options.apiPrefix || '/api/flows/robosats').replace(/\/+$/, '')
    const seen = new Set()
    ...
    for (const ep of endpoints) {
      ...
      if (seen.has(key)) throw new Error(`[robosats-flow] Duplicate endpoint: ${key}`)
      seen.add(key)
